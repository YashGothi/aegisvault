"""
Django settings for vault_project.
"""
...
ASGI_APPLICATION = 'vault_project.asgi.application'

# Database Configuration (PostgreSQL by default with SQLite fallback if PostgreSQL is not active)
...
# Check if PostgreSQL connection is desired vs SQLite fallback
...
if CORS_ALLOWED_ORIGINS_RAW:
    CORS_ALLOWED_ORIGINS = [orig.strip() for orig in CORS_ALLOWED_ORIGINS_RAW.split(',') if orig.strip()]
...
# Master Encryption Key for AES-256 Fernet Password Storage
ENCRYPTION_KEY = os.getenv('ENCRYPTION_KEY', 'ntnmMaYhcoCP9puA2Ir-dnfPxjIXW1yJVQHxQCMunYA=')
